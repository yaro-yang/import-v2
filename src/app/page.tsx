...
export default function HomePage() {
  ...
  const handleCreateNewRule = useCallback(async () => {
    ...
    try {
      ...
      if (data.success && data.data) {
        ...
        toast.success("AI 已完成分析，请确认并调整规则");
      } else {
        // AI 分析失败，打开空规则编辑器
        setEditingRule({
          fileType:
  ...
  const handleSubmit = useCallback(async () => {
    ...
    try {
      ...
      if (data.success) {
        ...
        setStep("submitted");
        toast.success(`成功提交 ${data.data.savedCount} 条运单！`);
      } else {
  ...
  return (
    <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
      {/* 页面标题 */}
      <div className="mb-8">
        <h1 className="text-2xl font-bold text-[#1d2129]">导入下单</h1>
        ...
        {[
          ...
          return (
            <div key={s.key} className="flex items-center gap-2">
              ...
              >
                {isActive && !isCurrent ? "✓" : i + 1} {s.label}
              </div>
            </div>
          );
        })}
      ...
      <div className="bg-white rounded-xl shadow-sm border border-[#e5e6eb] p-6">
        ...
        {step === "select-rule" && file && (
          <div className="animate-fade-in space-y-6">
            ...
            <div className="flex items-center gap-4 p-4 bg-[#f7f8fa] rounded-lg">
              <div className="w-10 h-10 rounded-lg bg-[#e8fafa] flex items-center justify-center text-[#0fc6c2] text-lg">
                📄
              </div>
        ...
        {step === "preview" && (
          <div className="animate-fade-in space-y-4">
            {/* 统计信息 */}
            <div className="flex items-center gap-6 p-4 bg-[#f7f8fa] rounded-lg">
              <div>
                <p className="text-xs text-[#86909c]">总数据</p>
                <p className="text-lg font-bold text-[#1d2129]">
                  {orders.length} 条
                </p>
              </div>
              <div>
                <p className="text-xs text-[#86909c]">解析耗时</p>
                <p className="text-lg font-bold text-[#0fc6c2]">
                  {formatTime(parseTime)}
                </p>
              </div>
              {errors.length > 0 && (
                <div>
                  <p className="text-xs text-[#86909c]">校验错误</p>
            ...
            {/* 操作按钮 */}
            <div className="flex justify-between items-center pt-4 border-t border-[#e5e6eb]">
        ...
        {step === "submitted" && (
          <div className="animate-fade-in text-center py-12">
            <div className="w-20 h-20 rounded-full bg-[#e8fafa] flex items-center justify-center mx-auto mb-4">
              <span className="text-4xl">✅</span>
            </div>
            <h2 className="text-xl font-bold text-[#1d2129] mb-2">
              提交成功！
            </h2>
            <p className="text-[#4e5969] mb-6">
              已成功提交{" "}
              <span className="font-bold text-[#0fc6c2]">{submittedCount}</span>{" "}
              条运单
            </p>
