    ...
    for (const g of groups) {
      for (const o of g.orders) {
        ...
        if (safeItems.length === 0) {
          rows.push({
            外部编码: g.externalCode, 收货门店: o.storeName || "", 收件人: o.recipientName || "",
            电话: o.recipientPhone || "", 地址: o.recipientAddress || "",
          ...
          for (const item of safeItems) {
            rows.push({
              外部编码: g.externalCode, 收货门店: o.storeName || "", 收件人: o.recipientName || "",
              电话: o.recipientPhone || "", 地址: o.recipientAddress || "",
              SKU编码: item.skuCode || "", SKU名称: item.skuName || "",
              数量: item.skuQuantity || 0, 规格: item.skuSpec || "", 备注: o.remark || "",
            });
          }
        }
      }
    ...
    <div className="space-y-6">
          ...
          <div key={i} className="bg-white rounded-2xl border border-gray-100 p-6 hover:shadow-md transition-shadow">
            <div className="flex items-center justify-between mb-3">
              ...
              <div className={`w-9 h-9 rounded-xl ${card.bg} flex items-center justify-center`}>
                <svg className={`w-5 h-5 ${card.color}`} fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d={card.icon} /></svg>
              </div>
            </div>
      ...
      <div className="bg-white rounded-2xl border border-gray-100 p-5">
        <div className="flex flex-wrap items-center gap-4">
          ...
          <button onClick={() => { setPage(1); loadOrders(); }} className="inline-flex items-center justify-center gap-2 px-10 py-3.5 text-sm font-medium text-white bg-gradient-to-r from-teal-500 to-cyan-500 rounded-xl hover:from-teal-600 hover:to-cyan-600 transition-all shadow-md shadow-teal-500/25 min-w-[88px] whitespace-nowrap">
            <svg className="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M21 21l-6-6m2-5a7 7 0 11-14 0 7 7 0 0114 0z" /></svg>
            查询
          </button>
          {selectedGroups.size > 0 && (
            <button onClick={handleBatchDelete} className="ml-auto px-5 py-3 text-sm font-medium text-white bg-rose-500 rounded-xl hover:bg-rose-600 transition-colors">
                ...
                <tr className="border-b border-gray-100 bg-gray-50/50">
                  <th className="py-4 px-5 w-10">
                    <input type="checkbox" checked={groups.length > 0 && selectedGroups.size === groups.length}
                      onChange={selectAll} className="accent-teal-500 w-4 h-4" />
                  </th>
                  <th className="text-left py-4 px-5 text-xs font-medium text-gray-400 uppercase tracking-wider w-36">外部编码</th>
                  <th className="text-left py-4 px-5 text-xs font-medium text-gray-400 uppercase tracking-wider w-36">收货门店</th>
                  ...
                  <th className="text-left py-4 px-5 text-xs font-medium text-gray-400 uppercase tracking-wider w-32 border-r border-gray-100">数量</th>
                  ...
                  return showRows.map((row, idx) => {
                    ...
                    return (
                      <tr key={`${group.id}-${idx}`} className="border-b border-gray-50 hover:bg-gray-50/30 transition-colors">
                          ...
                          <>
                            <td rowSpan={showRows.length} className="py-4 px-5 align-top">
                              <input type="checkbox" checked={selectedGroups.has(group.id)}
                                onChange={() => toggleSelect(group.id)} className="accent-teal-500 w-4 h-4" />
                            </td>
                            <td rowSpan={showRows.length} className="py-4 px-5 align-top">
                              <div className="font-mono text-sm font-semibold text-gray-700">{group.externalCode}</div>
                              {rows.length > 1 && (
                                <button onClick={() => toggleGroup(group.id)}
                                  className="text-xs text-teal-500 hover:text-teal-600 mt-1.5 font-medium">
                        ...
                        <td className="py-4 px-5 text-gray-600 text-sm truncate max-w-36">{o.storeName || "-"}</td>
                        <td className="py-4 px-5 text-gray-600 text-sm">{o.recipientName || "-"}</td>
                        ...
                        <td className="py-4 px-5 text-left text-gray-600 text-sm tabular-nums w-32 border-r border-gray-100">{item?.skuQuantity ?? "-"}</td>
                        ...
                        <td className="py-4 px-5 text-right text-gray-400 text-sm whitespace-nowrap">
                        ...
                        {isFirst && (
                          <td rowSpan={showRows.length} className="py-4 px-5 text-center align-top">
                            <button onClick={() => handleDelete(group.id)}
                              className="inline-flex items-center gap-1.5 px-3 py-1.5 text-sm text-gray-400 hover:text-rose-500 hover:bg-rose-50 rounded-xl transition-colors">
        ...
        <div className="flex items-center justify-between pt-2">
          ...
          <div className="flex items-center gap-3">
            <button onClick={() => setPage(p => Math.max(1, p - 1))} disabled={page <= 1}
              className="px-4 py-2 text-sm rounded-xl border border-gray-200 disabled:opacity-30 hover:border-teal-300 transition-colors">上一页</button>
            <span className="text-sm font-medium text-gray-600 px-2">{page}</span>
            <button onClick={() => setPage(p => Math.min(totalPages, p + 1))} disabled={page >= totalPages}
              className="px-4 py-2 text-sm rounded-xl border border-gray-200 disabled:opacity-30 hover:border-teal-300 transition-colors">下一页</button>
          </div>
        </div>
      )}
    </div>
